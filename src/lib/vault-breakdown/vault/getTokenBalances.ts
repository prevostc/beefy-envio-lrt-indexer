...
import { getBalanceSubgraphUrl, SUBGRAPH_PAGE_SIZE } from '../config';
...
export const getTokenBalances = async (
    ...
    `;
        ...
        const res = (await response.json()) as { data: QueryResult } | { errors: { message: string }[] };
        if ('errors' in res) {
            const errors = res.errors.map((e) => e.message).join(', ');
            throw new FriendlyError(`Subgraph query failed: ${errors}`);
        }

        const foundPositions = PARRALEL_REQUESTS_ARRAY.flatMap((i) => res.data[`tokenBalances${i}`] || []).map(
