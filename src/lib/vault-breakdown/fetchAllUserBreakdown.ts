...
export const getUserTVLAtBlock = async (
    ...
    const mergedInvestorPositionsByInvestorAndClmAddress: Record<string, (typeof investorPositions)[0]> = {};
    ...
    const vaults = vaultConfigs.filter(
        (vault) =>
            ...
            vault.reward_pools.some((pool) => vaultAddressWithActivePosition.includes(pool.reward_pool_address))
    );
